...
static const AVClass eval_class = { "Eval", vcn_av_default_item_name, NULL, LIBAVUTIL_VERSION_INT, offsetof(Parser,log_offset), offsetof(Parser,log_ctx) };
...
{
    return vcn_av_gettime() * 0.000001;
}
...
{
    AVExpr *d = vcn_av_mallocz(sizeof(AVExpr));
    ...
    p->s= strchr(p->s, '(');
    if (!p->s) {
        vcn_av_log(p, AV_LOG_ERROR, "Undefined constant or missing '(' in '%s'\n", s0);
        p->s= next;
        av_expr_free(d);
    ...
    if (*next == '(') { // special case do-nothing
        ...
        if ((ret = parse_expr(&d, p)) < 0)
            return ret;
        if (p->s[0] != ')') {
            vcn_av_log(p, AV_LOG_ERROR, "Missing ')' in '%s'\n", s0);
            av_expr_free(d);
            return AVERROR(EINVAL);
        }
        p->s++; // ")"
    ...
    if (p->s[0] != ')') {
        vcn_av_log(p, AV_LOG_ERROR, "Missing ')' or too many args in '%s'\n", s0);
        av_expr_free(d);
        return AVERROR(EINVAL);
    }
    p->s++; // ")"
    ...
    if (strmatch(next, "sinh"  )) d->a.func0 = sinh;
        ...
        vcn_av_log(p, AV_LOG_ERROR, "Unknown function in '%s'\n", s0);
        av_expr_free(d);
        return AVERROR(EINVAL);
    }
...
{
    AVExpr *e = vcn_av_mallocz(sizeof(AVExpr));
    if (!e)
        return NULL;
    e->type     =type   ;
    e->value    =value  ;
                  ...
                  int log_offset, void *log_ctx)
{
    Parser p = { 0 };
    AVExpr *e = NULL;
    char *w = vcn_av_malloc(strlen(s) + 1);
    char *wp = w;
    const char *s0 = s;
    int ret = 0;
    ...
    *wp++ = 0;
    ...
    if ((ret = parse_expr(&e, &p)) < 0)
        goto end;
    if (*p.s) {
        vcn_av_log(&p, AV_LOG_ERROR, "Invalid chars '%s' at the end of expression '%s'\n", p.s, s0);
        ret = AVERROR(EINVAL);
        goto end;
    }
    if (!verify_expr(e)) {
    ...
    e->var= vcn_av_mallocz(sizeof(double) *VARS);
    if (!e->var) {
        ret = AVERROR(ENOMEM);
        goto end;
    }
