...
const char *vcn_av_default_item_name(void *ptr)
{
    return (*(AVClass **) ptr)->class_name;
}

AVClassCategory vcn_av_default_get_category(void *ptr)
{
...
void vcn_av_log_format_line(void *ptr, int level, const char *fmt, va_list vl,
                        char *line, int line_size, int *print_prefix)
{
    vcn_av_log_format_line2(ptr, level, fmt, vl, line, line_size, print_prefix);
}

int vcn_av_log_format_line2(void *ptr, int level, const char *fmt, va_list vl,
                        char *line, int line_size, int *print_prefix)
{
    AVBPrint part[4];
    int ret;
...
void vcn_av_log_default_callback(void* ptr, int level, const char* fmt, va_list vl)
{
...
static void (*vcn_av_log_callback)(void*, int, const char*, va_list) = vcn_av_log_default_callback;
static void (*vcn_av_info_callback)(void*, int, int, const char*) = NULL;

int vcn_av_getloglevel() {
    return av_log_level;
}
void vcn_av_logx(void* avcl, int level, const char *fmt, ...)
{
    AVClass* avc = avcl ? *(AVClass **) avcl : NULL;
    va_list vl;
    va_start(vl, fmt);
    ...
    vcn_av_vlog(avcl, level, fmt, vl);
    va_end(vl);
}

void vcn_av_ll(void *avcl, int level, const char* file_name, const char* function_name, int line_num, const char *fmt, ...) {
    const int size = 512;
    char logbuffer[512];
    int print_prefix = 0;
    if(level >av_log_level)
    ...
    va_list vl;
    va_start(vl, fmt);
    vcn_av_log_format_line(avcl, level, fmt, vl, logbuffer, size, &print_prefix);
    va_end(vl);
    vcn_av_logx(avcl, level, "<%s %s %d> %s", file_name, function_name, line_num, logbuffer);
}
void vcn_av_vlog(void* avcl, int level, const char *fmt, va_list vl)
{
    void (*log_callback)(void*, int, const char*, va_list) = vcn_av_log_callback;
    if (log_callback)
        log_callback(avcl, level, fmt, vl);
}
void vcn_av_log_extern(void *avcl, info_callback callback, int type, int code, const char* file, const char* function, int line, const char *fmt, ...) {
    const int size = 512;
    char logbuffer[512];
    char retbuf[512];
    ...
    if(type != AV_LOG_ERROR) {
        vcn_av_logx(avcl, type, "<%s %s %d> %s\n", file, function, line, logbuffer);
    } else if (callback != NULL){
        callback(avcl, code, type, retbuf);
    }
}
void vcn_av_info_set_callback(void(*callback)(void*, int, int, const char*)) {
    vcn_av_info_callback = callback;
}
void vcn_av_info(void *avcl, int type, int code, const char* info) {
    if( vcn_av_info_callback != NULL ) {
        vcn_av_info_callback(avcl, type, code, info);
    }
}

int vcn_av_log_get_level(void)
{
    return av_log_level;
}

void vcn_av_log_set_level(int level)
{
    av_log_level = level;
}

void vcn_av_log_set_flags(int arg)
{
    flags = arg;
}

int vcn_av_log_get_flags(void)
{
    return flags;
}

void vcn_av_log_set_callback(void (*callback)(void*, int, const char*, va_list))
{
    vcn_av_log_callback = callback;
}

static void missing_feature_sample(int sample, void *avc, const char *msg,
                                   va_list argument_list)
{
    vcn_av_vlog(avc, AV_LOG_WARNING, msg, argument_list);
    vcn_av_log(avc, AV_LOG_WARNING, " is not implemented. Update your FFmpeg "
           "version to the newest one from Git. If the problem still "
           "occurs, it means that your file has a feature which has not "
           "been implemented.\n");
    if (sample)
        vcn_av_log(avc, AV_LOG_WARNING, "If you want to help, upload a sample "
