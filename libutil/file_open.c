...
static const AVClass file_log_ctx_class = {
    "TEMPFILE", vcn_av_default_item_name, NULL, LIBAVUTIL_VERSION_INT,
...
int avpriv_tempfile(const char *prefix, char **filename, int log_offset, void *log_ctx)
{
    ...
    if(!ptr)
        ptr= tempnam(".", prefix);
    *filename = vcn_av_strdup(ptr);
#undef free
    free(ptr);
#else
    size_t len = strlen(prefix) + 12; /* room for "/tmp/" and "XXXXXX\0" */
    *filename  = vcn_av_malloc(len);
#endif
    /* -----common section-----*/
    if (!*filename) {
        vcn_av_log(&file_log_ctx, AV_LOG_ERROR, "ff_tempfile: Cannot allocate file name\n");
...
#   endif
    ...
    if (fd < 0) {
        ...
        vcn_av_log(&file_log_ctx, AV_LOG_ERROR, "ff_tempfile: Cannot open temporary file %s\n", *filename);
        vcn_av_freep(filename);
        return err;
    }
    return fd; /* success */
}
