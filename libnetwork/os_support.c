                   ...
                   const struct addrinfo *hints, struct addrinfo **res)
{
    ...
    *res = NULL;
    sin  = vcn_av_mallocz(sizeof(struct sockaddr_in));
    if (!sin)
        return EAI_FAIL;
    ...
    ai = vcn_av_mallocz(sizeof(struct addrinfo));
    if (!ai) {
        vcn_av_free(sin);
        return EAI_FAIL;
    }
    ...
    *res            = ai;
    ...
    if (hints && (hints->ai_flags & AI_CANONNAME))
        ai->ai_canonname = h ? vcn_av_strdup(h->h_name) : NULL;
...
void ff_freeaddrinfo(struct addrinfo *res)
{
    ...
    vcn_av_freep(&res->ai_canonname);
    vcn_av_freep(&res->ai_addr);
    vcn_av_freep(&res);
}
