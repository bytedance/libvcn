...
#include "libutil/vcn_internal.h"
...
static int cache_open(VCNURLContext *h, const char *arg, int flags, AVDictionary **options)
{
    char *buffername;
    Context *c= h->priv_data;

    vcn_av_strstart(arg, "cache:", &arg);

    c->fd = avpriv_tempfile("ffcache", &buffername, 0, h);
    if (c->fd < 0){
        vcn_av_log(h, AV_LOG_ERROR, "Failed to create tempfile\n");
        return c->fd;
    }

    unlink(buffername);
    vcn_av_freep(&buffername);

    return vcn_url_open_whitelist(&c->inner, arg, flags, &h->interrupt_callback,
                                options, h->protocol_whitelist, h->protocol_blacklist, h);
}

static int add_entry(VCNURLContext *h, const unsigned char *buf, int size)
{
    Context *c= h->priv_data;
    int64_t pos = -1;
    int ret;
    ...
    if (pos < 0) {
        ret = AVERROR(errno);
        vcn_av_log(h, AV_LOG_ERROR, "seek in cache failed\n");
        goto fail;
    }
    c->cache_pos = pos;
    ...
    if (ret < 0) {
        ret = AVERROR(errno);
        vcn_av_log(h, AV_LOG_ERROR, "write in cache failed\n");
        goto fail;
    }
    c->cache_pos += ret;
    ...
    if (!entry ||
        ...
        entry = vcn_av_malloc(sizeof(*entry));
        ...
        if (entry_ret && entry_ret != entry) {
            ret = -1;
            vcn_av_log(h, AV_LOG_ERROR, "av_tree_insert failed\n");
            goto fail;
        }
    } else
        entry->size += ret;
...
static int cache_read(VCNURLContext *h, unsigned char *buf, int size)
{
    Context *c= h->priv_data;
    ...
    if (c->logical_pos != c->inner_pos) {
        r = vcn_url_seek(c->inner, c->logical_pos, SEEK_SET);
        if (r<0) {
            vcn_av_log(h, AV_LOG_ERROR, "Failed to perform internal seek\n");
            return r;
        }
        c->inner_pos = r;
    }

    r = vcn_url_read(c->inner, buf, size);
    if (r == 0 && size>0) {
        c->is_true_eof = 1;
...
static int64_t cache_seek(VCNURLContext *h, int64_t pos, int whence)
{
    Context *c= h->priv_data;
    int64_t ret;
    ...
    if (whence == AVSEEK_SIZE) {
        pos= vcn_url_seek(c->inner, pos, whence);
        if(pos <= 0){
            pos= vcn_url_seek(c->inner, -1, SEEK_END);
            if (vcn_url_seek(c->inner, c->inner_pos, SEEK_SET) < 0)
                vcn_av_log(h, AV_LOG_ERROR, "Inner protocol failed to seekback end : %"PRId64"\n", pos);
        }
        if (pos > 0)
            c->is_true_eof = 1;
...
resolve_eof:
    ...
    //cache miss
    ret= vcn_url_seek(c->inner, pos, whence);
...
static int cache_close(VCNURLContext *h)
{
    Context *c= h->priv_data;

    vcn_av_log(h, AV_LOG_INFO, "Statistics, cache hits:%"PRId64" cache misses:%"PRId64"\n",
           c->cache_hit, c->cache_miss);

    close(c->fd);
    vcn_url_close(c->inner);
