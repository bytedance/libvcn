...
#include "libutil/bprint.h"
#include "libutil/crc.h"
#include "libvcn/vcn_dict.h"
#include "libutil/intreadwrite.h"
...
static void *vcn_avio_child_next(void *obj, void *prev)
{
    AVIOContext *s = obj;
...
static const AVClass *vcn_avio_child_class_next(const AVClass *prev)
{
    return prev ? NULL : &vcn_url_context_class;
}
static int64_t vcn_avio_get_log_handle(void * ptr) {
    AVIOContext* s = ptr;
    return s->log_handle;
}
...
static const AVOption vcn_avio_options[] = {
...
const AVClass vcn_avio_class = {
    .class_name = "AVIOContext",
    .item_name  = vcn_av_default_item_name,
    .version    = LIBAVUTIL_VERSION_INT,
    .option     = vcn_avio_options,
    .child_next = vcn_avio_child_next,
    .child_class_next = vcn_avio_child_class_next,
    .get_log_handle = vcn_avio_get_log_handle,
};

static void fill_buffer(AVIOContext *s);
static int url_resetbuf(AVIOContext *s, int flags);

int vcn_io_init_context(AVIOContext *s,
...
AVIOContext *avio_alloc_context(
                  ...
                  int64_t (*seek)(void *opaque, int64_t offset, int whence))
{
    AVIOContext *s = vcn_av_mallocz(sizeof(AVIOContext));
    if (!s)
        return NULL;
    vcn_io_init_context(s, buffer, buffer_size, write_flag, opaque,
...
void vcn_io_fill(AVIOContext *s, int b, int count)
{
    while (count > 0) {
...
static inline int put_str16(AVIOContext *s, const char *str, const int be)
{
    ...
    while (*q) {
        ...
        continue;
invalid:
        vcn_av_log(s, AV_LOG_ERROR, "Invalid UTF8 sequence in avio_put_str16%s\n", be ? "be" : "le");
        err = AVERROR(EINVAL);
        if (!*(q-1))
            break;
    }
...
static void fill_buffer(AVIOContext *s)
{
    ...
    if (s->read_packet && s->orig_buffer_size && s->buffer_size > s->orig_buffer_size) {
        if (dst == s->buffer) {
            int ret = vcn_io_set_buf_size(s, s->orig_buffer_size);
            if (ret < 0)
                vcn_av_log(s, AV_LOG_WARNING, "Failed to decrease buffer size\n");
...
int vcn_io_read_size(AVIOContext *s, unsigned char *buf, int size)
{
...
int vcn_io_read_indirect(AVIOContext *s, unsigned char *buf, int size, const unsigned char **data)
{
...
int vcn_io_read_partial(AVIOContext *s, unsigned char *buf, int size)
{
    int len;
...
uint64_t vcn_io_read_varlen(AVIOContext *bc){
    uint64_t val = 0;
    int tmp;
...
static int io_read_packet(void *opaque, uint8_t *buf, int buf_size)
{
    ...
    return vcn_url_read(internal->h, buf, buf_size);
}

static int io_write_packet(void *opaque, uint8_t *buf, int buf_size)
{
    AVIOInternal *internal = opaque;
    return vcn_url_write(internal->h, buf, buf_size);
}

static int64_t io_seek(void *opaque, int64_t offset, int whence)
{
    AVIOInternal *internal = opaque;
    return vcn_url_seek(internal->h, offset, whence);
}

static int io_short_seek(void *opaque)
{
    AVIOInternal *internal = opaque;
    return vcn_url_get_short_seek(internal->h);
}
...
int vcn_io_fdopen(AVIOContext **s, VCNURLContext *h)
{
    ...
    buffer = vcn_av_malloc(buffer_size);
    if (!buffer)
        return AVERROR(ENOMEM);

    internal = vcn_av_mallocz(sizeof(*internal));
    if (!internal)
        goto fail;
    ...
    *s = avio_alloc_context(buffer, buffer_size, h->flags & AVIO_FLAG_WRITE,
    ...
    (*s)->protocol_whitelist = vcn_av_strdup(h->protocol_whitelist);
    if (!(*s)->protocol_whitelist && h->protocol_whitelist) {
        avio_closep(s);
        goto fail;
    }
    (*s)->protocol_blacklist = vcn_av_strdup(h->protocol_blacklist);
    ...
    (*s)->av_class = &vcn_avio_class;
    return 0;
fail:
    vcn_av_freep(&internal);
    vcn_av_freep(&buffer);
    return AVERROR(ENOMEM);
}
...
int ffio_ensure_seekback(AVIOContext *s, int64_t buf_size)
{
    ...
    buffer = vcn_av_malloc(buf_size);
    if (!buffer)
        return AVERROR(ENOMEM);

    memcpy(buffer, s->buffer, filled);
    vcn_av_free(s->buffer);
...
int vcn_io_set_buf_size(AVIOContext *s, int buf_size)
{
    uint8_t *buffer;
    buffer = vcn_av_malloc(buf_size);
    if (!buffer)
...
int vcn_io_rewind_with_probe_data(AVIOContext *s, unsigned char **bufp, int buf_size)
{
...
int vcn_io_open_whitelist(AVIOContext **s, const char *filename, int flags,
                        ...
                        )
{
    ...
    err = vcn_url_open_whitelist(&h, filename, flags, int_cb, options, whitelist, blacklist, NULL);
    if (err < 0) {
        //vcn_av_trace(NULL,err,"err:%d", err);
        return err;
		}
    err = vcn_io_fdopen(s, h);
    if (err < 0) {
        vcn_url_close(h);
...
int avio_open2(AVIOContext **s, const char *filename, int flags,
               const AVNetIOInterruptCB *int_cb, AVDictionary **options)
{
    return vcn_io_open_whitelist(s, filename, flags, int_cb, options, NULL, NULL);
}
...
int avio_close(AVIOContext *s)
{
    ...
    vcn_av_freep(&s->opaque);
    vcn_av_freep(&s->buffer);
    if (s->write_flag)
        vcn_av_log(s, AV_LOG_DEBUG, "Statistics: %d seeks, %d writeouts\n", s->seek_count, s->writeout_count);
    else
        vcn_av_log(s, AV_LOG_DEBUG, "Statistics: %"PRId64" bytes read, %d seeks\n", s->bytes_read, s->seek_count);
    vcn_av_opt_free(s);
    vcn_av_free(s);
    return vcn_url_close(h);
}
...
int avio_accept(AVIOContext *s, AVIOContext **c)
{
    int ret;
    AVIOInternal *internal = s->opaque;
    VCNURLContext *sc = internal->h;
    VCNURLContext *cc = NULL;
    ret = vcn_url_accept(sc, &cc);
    if (ret < 0)
        return ret;
    return vcn_io_fdopen(c, cc);
}

int avio_handshake(AVIOContext *c)
{
    AVIOInternal *internal = c->opaque;
    VCNURLContext *cc = internal->h;
    return vcn_url_handshake(cc);
}
...
static int dyn_buf_write(void *opaque, uint8_t *buf, int buf_size)
{
    ...
    if (new_allocated_size > d->allocated_size) {
        int err;
        if ((err = vcn_av_reallocp(&d->buffer, new_allocated_size)) < 0) {
            d->allocated_size = 0;
            d->size = 0;
            return err;
        }
...
static int url_open_dyn_buf_internal(AVIOContext **s, int max_packet_size)
{
    ...
    if (sizeof(DynBuffer) + io_buffer_size < io_buffer_size)
        return -1;
    d = vcn_av_mallocz(sizeof(DynBuffer) + io_buffer_size);
    if (!d)
        return AVERROR(ENOMEM);
...
int vcn_io_open_dyn_packet_buf(AVIOContext **s, int max_packet_size)
{
...
void vcn_io_free_dyn_buf(AVIOContext **s)
{
    uint8_t *tmp;
    if (!*s)
        return;
...
int vcn_io_open_null_buf(AVIOContext **s)
{
...
int vcn_io_close_null_buf(AVIOContext *s)
{
