...
#include "libutil/random_seed.h"
#include "libvcn/vcn_mem.h"
#include "libutil/md5.h"
#include "httpauth.h"
#include "libvcn/urldecode.h"
#include "libvcn/vcn_utils.h"
#include <string.h>
#include <stdio.h>
...
void ff_http_auth_handle_header(HTTPAuthState *state, const char *key,
                                const char *value)
{
    if (!vcn_av_strcasecmp(key, "WWW-Authenticate") || !vcn_av_strcasecmp(key, "Proxy-Authenticate")) {
        const char *p;
        if (vcn_av_stristart(value, "Basic ", &p) &&
            state->auth_type <= HTTP_AUTH_BASIC) {
            state->auth_type = HTTP_AUTH_BASIC;
            state->realm[0] = 0;
            state->stale = 0;
            vcn_parse_key_value(p, (ff_parse_key_val_cb) handle_basic_params,
                               state);
        } else if (vcn_av_stristart(value, "Digest ", &p) &&
                   state->auth_type <= HTTP_AUTH_DIGEST) {
            state->auth_type = HTTP_AUTH_DIGEST;
            memset(&state->digest_params, 0, sizeof(DigestParams));
            state->realm[0] = 0;
            state->stale = 0;
            vcn_parse_key_value(p, (ff_parse_key_val_cb) handle_digest_params,
                               state);
            choose_qop(state->digest_params.qop,
                       sizeof(state->digest_params.qop));
            if (!vcn_av_strcasecmp(state->digest_params.stale, "true"))
                state->stale = 1;
        }
    } else if (!vcn_av_strcasecmp(key, "Authentication-Info")) {
        vcn_parse_key_value(value, (ff_parse_key_val_cb) handle_digest_update,
                           state);
    }
}
                              ...
                              const char *method)
{
    ...
    vcn_data_to_hex(cnonce, (const uint8_t*) cnonce_buf, sizeof(cnonce_buf), 1);
    ...
    vcn_data_to_hex(A1hash, hash, 16, 1);
    A1hash[32] = 0;
    ...
    av_md5_final(md5ctx, hash);
    vcn_data_to_hex(A2hash, hash, 16, 1);
    A2hash[32] = 0;
    ...
    av_md5_final(md5ctx, hash);
    vcn_data_to_hex(response, hash, 16, 1);
    response[32] = 0;
    ...
    authstr = vcn_av_malloc(len);
    if (!authstr)
        return NULL;
    snprintf(authstr, len, "Authorization: Digest ");

    /* TODO: Escape the quoted strings properly. */
    vcn_av_strlcatf(authstr, len, "username=\"%s\"",   username);
    vcn_av_strlcatf(authstr, len, ", realm=\"%s\"",     state->realm);
    vcn_av_strlcatf(authstr, len, ", nonce=\"%s\"",     digest->nonce);
    vcn_av_strlcatf(authstr, len, ", uri=\"%s\"",       uri);
    vcn_av_strlcatf(authstr, len, ", response=\"%s\"",  response);

    // we are violating the RFC and use "" because all others seem to do that too.
    if (digest->algorithm[0])
        vcn_av_strlcatf(authstr, len, ", algorithm=\"%s\"",  digest->algorithm);

    if (digest->opaque[0])
        vcn_av_strlcatf(authstr, len, ", opaque=\"%s\"", digest->opaque);
    if (digest->qop[0]) {
        vcn_av_strlcatf(authstr, len, ", qop=\"%s\"",    digest->qop);
        vcn_av_strlcatf(authstr, len, ", cnonce=\"%s\"", cnonce);
        vcn_av_strlcatf(authstr, len, ", nc=%s",         nc);
    }

    vcn_av_strlcatf(authstr, len, "\r\n");
                                   ...
                                   const char *path, const char *method)
{
    ...
    if (state->auth_type == HTTP_AUTH_BASIC) {
        ...
        char *ptr, *decoded_auth = vcn_ff_urldecode(auth);
        ...
        char *username = vcn_ff_urldecode(auth), *password;
