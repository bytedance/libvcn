...
extern const AVClass vcn_avio_class;
...
static av_always_inline void vcn_io_wfourcc(AVIOContext *pb, const uint8_t *s)
{
 ...
 */
int vcn_io_rewind_with_probe_data(AVIOContext *s, unsigned char **buf, int buf_size);
 ...
 * resource referenced by the VCNURLContext h.
 * @note When the VCNURLContext h has been opened in read+write mode, the
...
int vcn_io_open_whitelist(AVIOContext **s, const char *url, int flags,
 ...
 * Close a null buffer.
 *
 * @param s an IO context opened by vcn_io_open_null_buf
